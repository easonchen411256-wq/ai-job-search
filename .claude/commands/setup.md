...
- **Job portals:** "This fork searches China-mainland public sources through the web-search based `china-public-job-search` skill (BOSS直聘、智联招聘、51job、猎聘、拉勾、脉脉、应届生、高校就业网、国聘、国企官网、企业官方招聘页、LinkedIn/外企官网、公开招聘公告)，plus the optional `linkedin-search` and `freehire-search` CLIs. Which additional portals fit your market?" **Then act on the answer:** if the user needs a local board that is not shipped, guide them to `/add-portal` (market-specific skills live in their fork). WebSearch/`site:` queries remain the fallback for portals without a CLI skill. Never bypass login, CAPTCHA, SMS/WeChat verification, or rate limits.
